...
void CACHE::update_partition_sizes(CACHE *cache)
{
    ...
    if (cache_type == IS_LLC && (((current_core_cycle[0] - last_update_cycle_cpu0) > 5000000) || ((current_core_cycle[1] - last_update_cycle_cpu1) > 5000000)))
    {
        ...
        uint64_t TOTAL_ACCESS_CPU1 = 0;

        for (uint32_t i = 0; i < NUM_TYPES; i++)
        {
            TOTAL_ACCESS_CPU0 += cache->sim_access[0][i];
        }

        for (uint32_t i = 0; i < NUM_TYPES; i++)
        {
            TOTAL_ACCESS_CPU1 += cache->sim_access[1][i];
        }
...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
    {
        ...
        //     set = get_set_edited_task2b(MSHR.entry[mshr_index].address, MSHR.entry[mshr_index].cpu);
        // }else{
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0))
    {
        ...
        uint32_t set;
        // if(cache_type == IS_LLC){
        //     set = get_set_edited_task2b(WQ.entry[index].address, WQ.entry[index].cpu);
        // }else{
        ...
        { // writeback miss (or RFO miss for L1D)
            ...
            else
            {
                ...
                uint32_t set, way;
                // if(cache_type == IS_LLC){
                //     set = get_set_edited_task2b(WQ.entry[index].address, WQ.entry[index].cpu), way;
                // }else{
                //     set = get_set(WQ.entry[index].address);
                // }
                set = get_set(WQ.entry[index].address);
...
void CACHE::handle_read()
{
    ...
    for (uint32_t i = 0; i < MAX_READ; i++)
    {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
        {
            ...
            uint32_t set;
            // if(cache_type == IS_LLC){
            //     set = get_set_edited_task2b(RQ.entry[index].address, RQ.entry[index].cpu);
            // }else{
...
void CACHE::handle_prefetch()
{
    ...
    for (uint32_t i = 0; i < MAX_READ; i++)
    {
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0))
        {
            ...
            uint32_t set;
            // if(cache_type == IS_LLC){
            //     set = get_set_edited_task2b(PQ.entry[index].address, PQ.entry[index].cpu);
            // }else{
...
void CACHE::operate()
{
    // if (cache_type == IS_LLC /*&& ooo_cpu[1].num_retired>= 10000000 && count<1*/)
    // {
...
uint32_t CACHE::get_set_edited(uint64_t address, uint32_t cpu)
{
    if (cache_type == IS_LLC)
    {
        uint32_t partition_size = NUM_SET / 2;
        uint32_t lower_bits = address & (partition_size - 1);
        return (cpu * partition_size) + lower_bits;
    }
    else
    {
#ifdef PUSH_DTLB_PB
...
void CACHE::fill_cache(uint32_t set, uint32_t way, PACKET *packet)
{
    ...
    //             << " | Addr: 0x" << std::hex << packet->address
    //             << " | EncryptedTag: 0x" << block[set][way].encrypted_tag
    //             << " | Epoch: " << std::dec << (int)block[set][way].epoch_id
    //             << " | Set: " << set
    //             << " | Way: " << way
    //             << std::endl;
    // }
...
void CEASERController::rekey_one_set(CACHE *cache, uint64_t current_cycle)
{
    ...
    for (uint32_t way = 0; way < 16; ++way)
    {
        ...
        cache->block[new_set][victim_way].lru = victim_lru;

        cache->lru_update(new_set, victim_way);
        blk.valid = 0;
    }
