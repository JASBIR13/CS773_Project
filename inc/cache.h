...
{
public:
    CEASERController* ceaser; 

    uint32_t cpu, self_eviction[2];
    const string NAME;
        ...
        : NAME(v1), NUM_SET(v2), NUM_WAY(v3), NUM_LINE(v4), WQ_SIZE(v5), RQ_SIZE(v6), PQ_SIZE(v7), MSHR_SIZE(v8)
    {
        ...
        // CEASER setup (only for LLC)
        if (cache_type == IS_LLC) {
            ceaser = new CEASERController(NUM_SET); 
        } else {
            ceaser = nullptr;
        }
    ...
    void handle_fill(),
        initialize(),
        update_partition_sizes(CACHE *cache),
        handle_writeback(),
        handle_read(),
        handle_prefetch(),
        flush_TLB(),
    ...
    void add_nonfifo_queue(PACKET_QUEUE *queue, PACKET *packet), //@Vishal: Updated from add_mshr
        ...
        lru_update_task2a(uint32_t set, uint32_t way),
        lru_update_task1(uint32_t cpu, uint32_t set, uint32_t way),
    ...
    uint64_t ceaser_encrypt_address(uint64_t address),
    ceaser_encrypt_address_with_eid(uint64_t address, uint8_t &eid);
    ...
    uint32_t get_set_edited(uint64_t address, uint32_t cpu),
    get_set_edited_task2b(uint64_t address, uint32_t cpu),
        get_set(uint64_t address),
        ...
        lru_victim_task1(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type),
        lru_victim_task2a(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
